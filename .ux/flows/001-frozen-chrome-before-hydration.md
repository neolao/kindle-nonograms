...
- **Data known only in the browser** (a puzzle already solved via `localStorage`, a badge already earned; and, since backlog item 072, the library's own saved color/status/sort filter selection, from a cookie) — the static default is the "nothing yet" state (banner hidden, badge hidden, every filter at "all"); a synchronous check at the very start of hydration — before the browser's first paint is perceived — corrects it if needed. This correction is never allowed to be visible as a transition; if it cannot be made invisible, it must not ship (see Exit & failure paths). Unlike the locale cookie (read via a blocking script placed in `<head>`, before any `<body>` content exists), the filter cookie cannot use that same technique — it needs to hide/reorder rows that don't exist yet at that point in parsing — so it is corrected the same way the solved badges already are: synchronously inside hydration itself (`.vibe/decisions/046-filter-restore-stays-synchronous-not-head-script.md`).
...
- [x] Loading the library always lands on page 1, with the correct Previous/Next/disabled state and total-page count computed at build time from the fixed puzzle count — with no flash or recalculation visible after first paint. Verified: `no-js-library.png` shows "1 / 2" correctly on a >25-puzzle library with JS entirely disabled. (Page number itself is never restored from a saved preference — every filter/sort change, including one restored from a cookie, always resets to page 1.)
- [x] Loading the library with a saved color/status/sort cookie (backlog item 072) shows that exact selection already applied — matching buttons already pressed, the list already narrowed/reordered — corrected synchronously during hydration before the page is perceived as painted, the same guarantee this flow already gives solved badges; a first visit or a cookie that fails to parse falls back to every filter at "all" with no error.
