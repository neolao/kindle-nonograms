...
button,.back-link{border:${BORDER_WIDTH.thin} solid ${COLORS.border};background:${COLORS.panel};color:${COLORS.text};padding:${SPACING_PX.xs}px ${SPACING_PX.md}px;min-height:${MIN_TAP_TARGET_PX}px;min-width:${MIN_TAP_TARGET_PX}px;}
button:focus,.back-link:focus{outline:${BORDER_WIDTH.thick} solid ${COLORS.focusOutline};}
button[aria-pressed="true"]{border-width:${BORDER_WIDTH.thick};border-color:${COLORS.amber};background:${COLORS.amberSoft};}
...
.dot-row i{width:6px;height:6px;background:${COLORS.muted};display:inline-block;}
...
.panel{background:${COLORS.panel};border:${BORDER_WIDTH.medium} solid ${COLORS.border};box-shadow:6px 6px 0 ${COLORS.panelEdge};}
`;
}
