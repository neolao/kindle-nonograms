...
Both page renderers share `htmlEscape.ts`: `escapeHtml` for puzzle names/ids in visible HTML, and `embedJson` for safely serializing data into a `<script type="application/json">` payload (escaping `<` so an embedded `</script>` sequence can never break out of the tag).
