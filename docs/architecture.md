...
`renderLibraryPage.ts` exposes `renderLibraryPage(summaries)`: a pure function producing the site's home page, a list of every puzzle (`PuzzleSummary` — just `id`/`name`/`width`/`height`, no solution data reaches this renderer at all) linking to `puzzles/<id>/`, or a plain empty-state message when the list is empty. Each row already contains its own hidden `.solved-badge` element for a later hydration script to reveal by toggling the `hidden` attribute, rather than injecting DOM at runtime — avoids a visible redraw flash on Kindle's slow e-ink refresh (see `.vibe/decisions/004-library-page-reserves-solved-badge-node.md`).

Both page renderers share one `htmlEscape.ts` helper for escaping puzzle names/ids into HTML-safe text.
