...
## Acceptance Criteria
- [ ] New workflow `.github/workflows/pr-check.yml` triggers on `pull_request` (opened/synchronize/reopened/closed) targeting `main`. The `closed` case is only needed by [[025-automatic-puzzle-preview-on-pull-requests]]'s cleanup step — this item's own validation steps only need to run for opened/synchronize/reopened.
- [ ] Runs the same steps as the existing `build` job in `deploy.yml` (checkout, setup Node 22, `npm ci`, `npm run lint`, `npm test`, `npm run build`) with no deploy step — this alone already exercises `loadPuzzleSources`, `createPuzzle`, and the content checks from [[023-puzzle-solvability-and-duplicate-validation]] against every puzzle file the PR adds or changes.
- [ ] The check's pass/fail status shows up in the PR's checks list (standard GitHub Actions behavior once the workflow triggers correctly — no extra step needed).
- [ ] `permissions:` on the workflow are scoped to `contents: read` only, since this job neither deploys nor writes anything.
