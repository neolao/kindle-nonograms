...
## Acceptance Criteria
- [ ] `createPuzzle` builds a valid `Puzzle` from well-formed input
- [ ] `createPuzzle` throws when width/height are not positive integers
- [ ] `createPuzzle` throws when `cells` shape doesn't match width×height, or a cell's color index is out of the palette's range
- [ ] `createPuzzle` throws when `id` or `name` is empty
