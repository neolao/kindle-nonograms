...
## Acceptance Criteria
- [x] `saveProgress` reports success/failure back to its caller instead of swallowing every error silently.
- [x] The first time a save fails during a session, the player sees a one-time, dismissible warning ("Progress can't be saved on this device").
- [x] The warning doesn't repeat on every subsequent failed save in the same session.
- [x] Existing successful-save behavior is unchanged.
