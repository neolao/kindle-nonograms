...
## Acceptance Criteria
- [x] Both puzzle files have a human-readable `name`, matching the naming style of the other puzzles converted in the same batch.
- [x] The `id` field (filename) is unchanged — only `name` is edited.
- [x] `npm test` (including the puzzle-content validation tests) still passes.
