...
## Acceptance Criteria
- [x] Each palette swatch's `aria-label` includes something that differentiates it from its siblings (index and/or hex value), e.g. "Select color 2 (#3388cc)".
- [x] Each color input's `aria-label` is differentiated the same way.
- [x] Labels stay correct after adding/removing/reordering palette colors.
