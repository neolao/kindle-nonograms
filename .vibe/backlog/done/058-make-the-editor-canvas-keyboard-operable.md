...
## Acceptance Criteria
- [x] Canvas cells are reachable via keyboard (e.g. arrow-key navigation within the grid).
- [x] Enter/Space on a focused cell paints/erases it, per the current mode, matching click behavior.
- [x] Each cell exposes a state-describing `aria-label` (e.g. current color name or "empty").
