...
## Acceptance Criteria
- [x] A solved puzzle's link accessible name includes its solved status (e.g. via `aria-label` on the `<a>`, updated wherever the badge is revealed).
- [x] An unsolved puzzle's link accessible name is unaffected.
- [x] The visible badge itself is unchanged.
