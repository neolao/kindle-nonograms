...
## Acceptance Criteria
- [x] On a large puzzle at a narrow (Kindle-width) viewport, clue/grid text never renders below a legible minimum size, even if that means the grid itself needs horizontal scroll or a lower effective content width.
- [x] Smaller puzzles' existing fit-to-viewport behavior is unchanged.
- [x] No puzzle's clues/cells get clipped by `.grid-wrapper`'s `overflow:hidden` fail-safe as a result of this change.
