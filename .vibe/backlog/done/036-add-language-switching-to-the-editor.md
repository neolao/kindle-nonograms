...
## Acceptance Criteria
- [x] `hydrateEditorPage.ts` applies the resolved locale (saved cookie, else browser language, else English) to every `[data-i18n]` element on the page, same mechanism as the library/puzzle pages.
- [x] The editor page shows a language switcher, consistent in placement/behavior with the library page's.
- [x] Switching language on the editor persists the choice (same cookie) and is honored on other pages afterward.
- [x] Every already-`data-i18n`-tagged element on the editor's default markup (baked by item "frozen default chrome") retranslates correctly.
