...
## Acceptance Criteria
- [x] A generic `button:disabled` rule (reduced opacity, no box-shadow) applies across the app, matching the one already used for pagination buttons.
- [x] The disabled remove-color button visibly reads as non-interactive.
- [x] No unintended visual change to any other, currently-enabled button.
